...
let audio;
let isPlaying = false;
let currentSentenceStartTime = 0; // Thời gian bắt đầu của câu hiện tại
let isTyping = false; // Biến để theo dõi trạng thái nhập liệu
let currentSpeed = 1.0;
let notes = []; // Mảng lưu trữ các note
...
function playAudio() {
    ...
    fetch(`/get_sentence?index=${currentIndex}`)
...
function checkAnswer() {
    ...
    fetch('/check_answer', {
        ...
        .then(data => {
            ...
            detailDiv.textContent = data.detail; // Hiển thị kết quả chi tiết
            ...
            if (data.correct) {
                ...
                // Tự động phát audio của câu tiếp theo từ vị trí bắt đầu của câu đó
                playAudio();
            } else {
...
function fetchDataForCurrentIndex() {
    return fetch(`/get_sentence?index=${currentIndex}`)
        .then(response => response.json())
        .then(data => {
            return data;
        });
