...
function playAudio() {
    ...
    fetch(`/get_sentence_shortstories?index=${currentIndex}`)
...
function checkAnswer() {
    ...
    fetch('/check_answer_shortstories', { // Sử dụng endpoint của Conversation
        method: 'POST',
        headers: {
...
function fetchDataForCurrentIndex() {
    return fetch(`/get_sentence_shortstories?index=${currentIndex}`)
        .then(response => response.json())
        .then(data => {
            return data;
        });
